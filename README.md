...
- 受控自我改进：本地保存显式偏好、任务经验和失败反思；技能候选必须通过安全评测并由用户批准，支持版本恢复。
...
在设置的 `Agent` 页面可以开启或关闭长期记忆、经验库、失败反思、技能候选生成和自动安全评测。

- 只有“请记住”“我偏好”“以后默认”等明确表达会自动写入长期记忆。
- 已完成任务会保存结果和脱敏后的工具步骤，失败任务会保存原因与下一次处理策略。
- 成功的工具任务可以生成声明式技能候选，但候选不会自动执行。
- 技能必须通过工具 schema、敏感信息、危险命令和风险等级检查，再由用户手动批准。
- 已批准技能只是 Agent 的规划参考，电脑操作仍然经过原有权限模式和敏感操作确认。
- 每次学习或管理操作都会增加数据版本并创建恢复点，可以在设置中查看记录或回滚最近一次改动。

完整说明见 [受控自我改进机制](docs/controlled-learning.md)。
...
Settings -> Agent contains separate controls for long-term memory, task experience, failure reflection, skill candidate generation, and automatic safety evaluation.

- Only explicit preference statements such as "remember", "I prefer", and "always use" are learned automatically.
- Completed tasks store outcomes and redacted tool steps. Failed tasks also store a cause and safer next strategy.
- Successful tool tasks may create declarative skill candidates. Candidates never execute automatically.
- A candidate must pass tool-schema, secret, dangerous-command, and risk checks, then receive explicit user approval.
- Approved skills are planning context only. Existing authorization modes and sensitive-action confirmations still apply.
- Every learning mutation increments the data revision and creates a local restore point for rollback.

See [Controlled self-improvement](docs/controlled-learning.md) for the full data and safety model.
